...
declare namespace ymaps {
    ...
    type GeocodingProviderKey = "yandex#map" | "yandex#search" | string;
    ...
    namespace control {
        ...
        interface ISearchControlParameters {
            ...
            options?: {
                ...
                provider?: IGeocodeProvider | GeocodingProviderKey;
    ...
    namespace option {
        /**
         * Менеджер опций.
         * Позволяет задавать и получать значения опций по строковому ключу,
         ...
         * Значением опции `"preset"` может быть хэш вида `{"имя опции": "значение опции"}`,
         * либо строковый идентификатор хэша опций в хранилище {@link option.presetStorage}.
         * Данный хэш опций может также содержать поле с именем `"preset"`,
         * что позволяет наследовать значения опций от других наборов.
         * 
         * При поиске значения в иерархии сначала проверяются собственные опции,
         * затем опции заданные с помощью ключа `"preset"`,
         * после чего происходит обращение к родителю, если он есть.
         * 
         * Для отслеживания изменений некоторых опций можно использовать {@link Monitor}.
         * 
         * - Используется классом {@link Map}
         * - Используется классом {@link control.Manager}
         * - Используется классом {@link GeoObject}
         * - Используется классом {@link Circle}
         * - Используется классом {@link objectManager.ClusterCollection}
         * - Используется классом {@link objectManager.ObjectCollection}
         * 
         * Они должны определить интерфейсы для субъектов которые управляются этим классом.
         */
        class Manager<ItemOptions extends IGeoObjectOptions> implements IOptionManager {
             ...
             */
            set<K extends keyof ItemOptions = keyof ItemOptions>(key: K, value?: ItemOptions[K]): this;
            set(key: ItemOptions)
    ...
    class Clusterer implements IChildOnMap, ICustomizable, IEventEmitter, IParentOnMap {
        ...
        getObjectState(geoObject: IGeoObject): { isShown: boolean; cluster: objectManager.ClusterData; isClustered: boolean };
    ...
    class GeoObject<T = IGeometry, TargetGeometry> implements IGeoObject<T> {
        constructor(feature?: IFeatureData, options?: IGeoObjectOptions);
     ...
     * - Опции {@link Balloon} с префиксом `balloon`.
     * - Опции {@link geoObject.Hint} геообъекта с префиксом `hint`.
     * - Опции редактора геометрии геообъекта с префиксом `editor`.
     * Тип редактора и перечень доступных опций зависит от типа геометрии геообъекта.
     * Смотрите описание классов {@link geometryEditor.LineString}, {@link geometryEditor.Polygon}, {@link geometryEditor.Point}.
     * - Опции геометрии могут задаваться без префикса.
     * Смотрите описание классов {@link IGeometry} геометрий {@link geometry.Point}, {@link geometry.Polygon} и др.
     */
    interface IGeoObjectOptions extends ICircleOptions, IBalloonOptionsWithBalloonPrefix {
        ...
        iconContentSize?: number[] | undefined;
        /**
         * Прямоугольная область (указывается левый верхний и правый нижний углы),
         * которая будет вырезана из исходного графического файла и смасштабирована под размеры иконки
         * (например для использования спрайтов).
         * По умолчанию исходная картинка подставляется полностью.
         */
        iconImageClipRect?: number[][] | undefined;
        /** 
         * URL графического файла иконки.
         * Применяется только в комбинации с макетами (`iconLayout`) `'default#image'` и `'default#imageWithContent'`.
         */
        iconImageHref?: string | undefined;
        /** Пиксельный сдвиг изображения иконки внутри родительского элемента. */
        iconImageOffset?: number[] | undefined;
        /**
         * Фигура активной области.
         * Если не задана, то автоматически будет рассчитана прямоугольная фигура на основе размера и смещения картинки.
         * Координаты геометрии фигуры отсчитываются от точки привязки.
         * Применяется только в комбинации с макетами (`iconLayout`) `'default#image'` и `'default#imageWithContent'`.
         */
        iconImageShape?: IShape | null | undefined;
        /** Размер иконки в пикселях. */
        iconImageSize?: number[] | undefined;
        /** Макет иконки. (Тип: конструктор объекта с интерфейсом {@link ILayout} или его ключ в хранилище). */
        iconLayout?: string | IClassConstructor<ILayout> | undefined;
        ...
        /**
         * Прямоугольная область (указывается левый верхний и правый нижний углы),
         * которая будет вырезана из исходного графического файла и смасштабирована под размеры тени
         * (например для использования спрайтов).
         * По умолчанию исходная картинка подставляется полностью.
         */
         ...
         * Применяется только в комбинации с макетами (`iconShadowLayout`) `'default#image'` и `'default#imageWithContent'`.
         */
        ...
        /** Макет тени иконки. (Тип: конструктор объекта с интерфейсом {@link ILayout} или его ключ в хранилище). */
        ...
        /**
         * Ключ-идентификатор из хранилища {@link ymaps.overlay.storage} или класс оверлея.
         * Функция-генератор принимает три параметра:
         * - geometry: {@link IPixelLineStringGeometry} - непосредственно пиксельная геометрия;
         * - data: {@link IDataManager} или `object` - данные оверлея;
         * - options: `object` - опции оверлея.
         * 
         * Функция возвращает объект {@link vow.Promise}.
         */
        ...
        /** Ключ предустановленных опций геообъекта. Список ключей содержится в описании {@link option.presetStorage}. */
        preset?: PresetKey | undefined;
        /**
         * Ключ-идентификатор из хранилища {@link ymaps.overlay.storage} или класс оверлея.
         * Функция-генератор принимает три параметра:
        ...
        /**
         * true — при перетаскивании объекта выставлять курсор «сжатая рука» и на объект, и на карту,
         * false — только на сам объект.
         */
    ...
    interface IMapOptions {
        ...
        searchControlProvider?: IGeocodeProvider | GeocodingProviderKey;
    ...
    interface IEvent<OriginalEvent = {}, TargetGeometry> {
         ...
         * С помощью этого метода можно получать доступ как к свойствам исходного события,
    ...
    type FeatureCollection<Item extends IFeatureData> = {
        type: "FeatureCollection"
        feathures: Item[]
    }

    /** Гео-объедк (метка, круг, линия и пр.) */
    interface IFeatureData {
        ...
        geometry: {
            ...
            type: "Point" | "LineString" | "Cicrle" | "Polygon",
            /** координаты объекта. Следует задавать в той последовательности, которая указана в параметре [coordorder](https://yandex.ru/dev/maps/jsapi/doc/2.1/dg/concepts/load.html#load) (если параметр не задан, используется последовательность «широта, долгота»). */
            coordinates: [number, number] | [[number, number], [number, number]] | [number, number][],
            /**  (только для объекта с типом «Circle») – радиус круга в метрах. */
            radius?: number
        }
        /**
         ...
         */
        properties?: IDataManager | object | undefined
        /** Опции объекта (например, стиль метки или цвет линии). Список доступных опций описан в классе {@link GeoObject}. */
        options: {
    ...
    /**
     * Инструмент для добавления большого числа объектов на карту.
     * С его помощью на карту можно добавить тысячи меток, линий и других объектов без потери производительности.
     * 
     * - Примечание. В ObjectManager отсутствует возможность перетаскивать объекты на карте, то есть опция draggable не поддерживается.
     * Также не поддерживается режим редактирования объектов.
     * 
     * На вход менеджеру подается JSON-описание объектов.
     * Оно должно быть сформировано разработчиком.
     * В описании необходимо указать геометрию объектов, их координаты и другую произвольную информацию
     * (например, названия организаций, часы работы и пр.).
     * На основе JSON-описания менеджер создаст оверлеи объектов (то есть их визуальное представление) и отобразит их на карте.
     * Оверлеи будут созданы только для тех объектов, которые попадают в текущую область показа карты.
     */
    class ObjectManager<ItemType extends IFeatureData> implements ICustomizable, IEventEmitter, IGeoObject, IParentOnMap {
        constructor(options: IObjectManagerOptions);

        clusters: objectManager.ClusterCollection<ObjectManager>;

        events: IEventManager;

        geometry: IGeometry | null;

        objects: objectManager.ObjectCollection<ItemType, IGeoObjectOptions>;
        ...
        add(objects: IFeatureData | FeatureCollection | (IFeatureData | FeatureCollection)[] | string): this;
        ...
        getObjectState(id: string): { found: boolean; isShown: boolean; cluster?: objectManager.ClusterData | undefined; isClustered: boolean; isFilteredOut: boolean };
    ...
    namespace objectManager {
        ...
        type ClusterData = {
            ...
            number: number
            type: "Cluster"
            feathures?: IFeatureData[]
            geometry: { type: string, coordinates: [number, number] }
            properties: {
                geoObjects: IFeatureData[];
                [k: string]: any
            }
            options: object
        }
        ...
        /**
         * - Используется классом {@link ObjectManager} для хранения всякой всячины
         * - Используется классом {@link LoadingObjectManager} для хранения всякой всячины
         */
        class ClusterCollection<ManagerType, ItemType, ItemOptionsType> implements ICustomizable, IEventEmitter {
            balloon: Balloon;
            ...
            options: option.Manager<ItemOptions>;

            state: data.Manager;

            each(callback: (object: ClusterData) => void, context?: object): void;

            getAll(): object[];

            /** Возвращает объект-кластер c указанным идентификатором или null, если такого кластера не существует. */
            getById(id: string): ClusterData | null;

            getIterator(): IIterator;

            getLength(): number;

            getObjectManager(): ManagerType;

            setClusterOptions(objectId: string, options: ItemOptionsType): ObjectCollection<ItemType, ItemOptionsType>;
        }
        ...
        /**
         * - Используется классом {@link ObjectManager} для кранения чего-то
         * - Используется классом {@link LoadingObjectManager} для кранения чего-то
         * - Неявно используется классом {@link ClusterCollection} для изменения элементов коллекции
         */
        class ObjectCollection<ItemType extends IFeatureData, ItemOptionsType extends IGeoObjectOptions> implements ICollection, ICustomizable {
            options: option.Manager<ItemOptionsType>;

            events: IEventManager<Item>;

            /** Метод полностью дублирует логику {@link ObjectManager.add}. */
            add(objects: Item | FeatureCollection<Item> | (Item | FeatureCollection<Item>)[] | string): this;

            each(callback: (object: IFeatureData) => void, context?: object): void;
            ...
            /**
             * Метод, позволяющий динамически обновлять опции объекта.
             * Метод следует использовать в случаях, когда вы хотите,
             * чтобы новые опции мгновенно применились к отображению объекта на карте.
             * Обратите внимание, менеджер не учитывает опцию 'visible'.
             * 
             * @param objectId Идентификатор целевого объекта, которому нужно задать опции.
             * @param options Новые опции целевого объекта.
             */
            setObjectOptions(objectId: number, options: ItemOptionsType): ObjectCollection<ItemType, ItemOptionsType>;
        }

        interface LoadingObjectManagerOptions extends IClustererOptions /** , IClusterPlacemarkOptions, IGeoObjectOptions */ {
            ...
            /** Флаг, разрешающий создавать оверлеи для объектов синхронно. Обратите внимание, что при синхронном создании оверлея нужно самостоятельно обеспечить загрузку нужного класса, реализующего интерфейс {@link IOverlay}. По умолчанию оверлеи создаются асинхронно, при этом класс оверлея загружается по требованию. */
        ...
        /**
         * Инструмент для добавления большого числа объектов на карту (например, меток, кругов, линий).
         * В отличие от {@link ObjectManager}, `LoadingObjectManager` сам производит загрузку данных с сервера,
         * причем только для тех объектов, которые попадают в видимую область карты
         * ({@link ObjectManager} работает только с теми объектами, которые были предварительно загружены на сторону клиента).
         * - Примечание. В `LoadingObjectManager` отсутствует возможность перетаскивать объекты на карте, то есть опция `draggable` не поддерживается.
         * Также не поддерживается режим редактирования объектов.
         * 
         * LoadingObjectManager принимает на вход JSON-описание объектов.
         * JSON-описание должно содержать идентификаторы объектов, их координаты,
         * а также различные свойства,например содержимое балунов.
         * На основе JSON-описания менеджер создаст `оверлеи` объектов (их визуальное представление) и отобразит их на карте.
         * 
         * `Оверлеи` — это визуальное представление объектов на карте.
         * Для их обработки требуется меньшее количество ресурсов, чем для обработки геообъектов — экземпляров класса {@link GeoObject}.
         */
        class LoadingObjectManager<ItemType extends IFeatureData> implements ICustomizable, IEventEmitter, IGeoObject, IParentOnMap {
            /**
             * @param urlTemplate шаблон URL данных. Поддерживаются специальные конструкции по аналогии с {@link Layer}. Также поддерживаются подстановки:
             ...
             * @param options Опции.
             * - Можно задавать все опции, указанные в описании {@link Clusterer}, за исключением опций `hasBalloon` и `hasHint`.
             * - Опции для кластеров задаются с префиксом `cluster`. Список опций указан в описании класса {@link ClusterPlacemark};
             * - Опции для одиночных объектов задаются с префиксом `geoObject`. Список опций определен в классе {@link GeoObject}. Обратите внимание, менеджер не учитывает опцию `'visible'`.
             */
            ...
            clusters: ClusterCollection<LoadingObjectManager<ItemType>, ItemType, IGeoObjectOptions>;
    
            objects: ObjectCollection<ItemType, IGeoObjectOptions>;
