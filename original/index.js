...
let getFilter_stage = (app.getUrlParameter('stages') || '').split(',').map(t => +t);
/** Url deserialized param 'work_type' */
let getFilter_work_type = (app.getUrlParameter('work_type') || '').split(',').map(t => +t);
/** Url deserialized param 'laststages' */
let getFilter_laststage = (app.getUrlParameter('laststages') || '').split(',').map(t => +t);
/** Url deserialized param 'sectors' */
let getFilter_sector = (app.getUrlParameter('sectors') || '').split(',').map(t => +t);
/** Url deserialized param 'fos' */
let getFilter_fo = (app.getUrlParameter('fos') || '').split(',').map(t => +t);
/** Url deserialized param 'regions' */
let getFilter_region = (app.getUrlParameter('regions') || '').split(',').map(t => +t);
/** Url deserialized param 'gos' */
let getFilter_gos = (app.getUrlParameter('gos') || '').split(',').map(t => +t);
/** Url deserialized param 'cost' */
let getFilter_cost = (app.getUrlParameter('cost') || '').split(',').map(t => +t);
/** Url deserialized param 'countries' */
let getFilter_country = (app.getUrlParameter('countries') || '').split(',').map(t => +t);
/** Url deserialized param 'cost' */
let getFilter_inv = (app.getUrlParameter('cost') || '').split(',').map(t => +t);
/** Url deserialized param 'x' */
let get_x = app.getUrlParameter('x') || null;
/** Url deserialized param 'y' */
let get_y = app.getUrlParameter('y') || null;
/** Url deserialized param 'clean_storage' */
let reloadStorage = app.getUrlParameter('clean_storage') || null;
/** Url deserialized param 'folders' */
let firstLoadFolders = (app.getUrlParameter('folders') || '').split(',').map(t => +t);
/** Url deserialized param 'layers' */
let firstLoadLayers = (app.getUrlParameter('layers') || '').split(',').map(t => t);
...
$(document).ready(function () {
        ...
        if (type === 'cluster') {
            ...
            objectManager.clusters.each((feathureCollection) => {
                ...
                for (const feature of feathureCollection.properties.geoObjects) {
                    if ((get_project_id && (+feature.id !== +get_project_id)) && (clickedObjectId != feathureCollection.id))
                        continue;
                    foundInCluster = true;
            ...
            zonesModel?.objectManager?.objects.each((object) => {
                if ((object.id == +(get_zone_id || '')) && balloon && !balloon.isOpen() && needOpenBaloon) {
                    needOpenBaloon = false;
                    balloon.open(object.geometry.coordinates, '<div style="margin: 0 0 5px;font-size: 120%;font-weight: 700;">'
    ...
    $('#filter_preview_cnt').append($('<div>', { 'id': 'search_screen_summ_cnt' })
        .append(searchBoxCnt)).show();
    if (isDemo)
    ...
    }).on('click', '#save_filters_button', function () {
        appLoadScreen.loading();
    ...
    ymaps.ready(function () {
        ...
        MyClusterIconContentLayout = ymaps.templateLayoutFactory.createClass('{% if properties.geoObjects.length > 100 %}'
            + '<div style="color:FF00FF;font-size:10px;">99+</div>'
            + '{% else %}'
            + '<div style="color:FF00FF;font-size:11px;">{{ properties.geoObjects.length }}</div>'
            + '{% endif %}');
        MyClusterIconContentLayoutHover = ymaps.templateLayoutFactory.createClass('{% if properties.geoObjects.length > 100 %}'
        ...
        function onAddCluster(e) {
            ...
            clusterIds.forEach(id => {
                if (allSeen.includes(id))
                    counter++;
            });
            let type = 'normal';
            if (counter == clusterIds.length)
                type = 'visited';
        ...
        function onObjectClick(e) {
            ...
            if (ymaps.cp_instance)
                ymaps.cp_instance.addIdsToClicked([object.id]);
        ...
        function onClusterClick(e) {
            ...
            // запишем ид для аналитики
            if (ymaps.cp_instance)
                ymaps.cp_instance.addIdsToClicked(featuresIds);
        }
    ...
    function setMapPoint(type, coordinates, preset, color, caption) {
        ...
        const searchMark = new type([coordinates[0], coordinates[1]], { hasBalloon: false, iconCaption: caption }, { preset: preset, iconCaptionMaxWidth: '250', iconColor: color });
    ...
    function loadProdAddressPanel(companyProdAddresses) {
        ...
        if (companyProdAddresses) {
            companyProdAddresses.forEach((this_wh, i) => {
                ...
                $('<div>', { class: 'wh__panel_line' })
                    ...
                    .append($('<div>', { class: 'wh__panel_name' }).html(typeName + editImg).data(data), $('<div>', { class: 'wh__panel_address' }).html(this_wh.prod_address), $('<div>', { class: 'wh_cnt' }).append($('<input>', { type: 'text', class: 'form-control mt-1 mb-3 form-control-sm wh__radius_input', placeholder: radiusKm })
                    .data(data), isAdmin
        ...
        if (!isGuest) {
            ...
            wh_control_panel.empty()
                .appendTo(map_div)
                .append($('<div>', { class: 'wh__panel_button ' + app.languageLocale, id: 'slideAddressCnt' })
                .html((app.languageLocale === 'en' ? 'Locations' : 'Адреса') + '<i class="wh-panel-toggler transition-all"></i>'), $('<div>', { class: 'wh_panel_subcontainer' })
                .append($('<div>', { class: 'wh__panel_header' })
                .html('<h4> ' + myAddresses + '</h4>'), $('<div>', { class: 'wh__panel_body' })
                .append(wh_panelBody, $('<div>')
                .html('<span class="' + spanClass + '" ' + dataString + '><b class="btn btn-sm btn-default">+ ' + addLocation + '</b></span>'), showProducts)));
            folders_control_panel.empty()
                .appendTo(map_div)
                .append($('<div>', { class: 'wh__panel_button', id: 'slideFoldersCnt' })
                .html((app.languageLocale === 'en' ? 'Folders' : 'Папки') + '<i class="wh-panel-toggler transition-all"></i>'), $('<div>', { class: 'wh_panel_subcontainer' })
                .append($('#folders_container'), $('<div>', { class: 'wh__panel_header wh__panel_header2' })
                .html('<h5> ' + seenChbx + '</h5>'), $('#seen_checkbox_container')));
        }
    }
    ...
    function addWHToTheMap(companyProdAddresses, Placemark) {
        ...
        companyProdAddresses.forEach((addr, index) => {
            const this_wh_pm = new Placemark([addr.map_x, addr.map_y], {
                hasBalloon: true,
...
function applyFilter(callbacks = []) {
    ...
    if (projects_to_map && projects_to_map.length > 0) {
        ...
        if (onlyFolders) {
            if (!selectedFoldersProjects.length)
                ...
                isFoldersOk = selectedFoldersProjects.includes(elem.id);
            isOk = isFoldersOk;
        }
        if (excludeFolders) {
            if (!selectedFoldersProjects.length)
...
function setLVRV(numl, numr, type, isClicked = false) {
    ...
    let nl = String(numl);
    let nr = String(numr);
    if (!nl || nl == undefined || nl == 'undefined')
        nl = 0;
    if (!nr || nr == undefined || nr == 'undefined')
        nr = 0;
...
function getCircle(coords, radius, color = 'rgba(158, 122, 128, 0.27)') {
    return new ymaps.Circle([coords, radius * 1000], { balloonContent: radius + ' ' + km, hintContent: radius + ' ' + km }, { draggable: false, fillColor: color, strokeColor: '#242424', strokeOpacity: 0.8, strokeWidth: 1 });
}
...
function addPlacemark({ map_x, map_y, iconImageHref, type, id }) {
    const dot = new ymaps.Placemark([map_x, map_y], {
        hasBalloon: true,
        balloonContentHeader: '',
        balloonContentBody: '',
        iconCaption: ''
    ...
    if (type === 'project')
        dot.events.add(['click'], () => placemarkClick(id));
    else
        dot.events.add(['click'], () => placemarkCompanyClick(id));
    return dot;
}
